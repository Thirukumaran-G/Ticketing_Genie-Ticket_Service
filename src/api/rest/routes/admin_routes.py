...
def _get_token(request: Request) -> str:
    """Extract raw Bearer token — forwarded to auth-service for tier/product lookups."""
    return request.headers.get("Authorization", "").removeprefix("Bearer ").strip()


# ── Reference data from auth-service (UI dropdowns) ──────────────────────────
# Admin token is forwarded so auth-service enforces its own role check.
# These populate tier_id / product_id pickers in the admin dashboard.

@router.get(
    "/tiers",
    summary="List tiers from auth-service — for SLA rule and severity map dropdowns",
)
...
@router.get(
    "/products",
    summary="List products from auth-service — for product config and report dropdowns",
)
...
@router.put(
    "/email-config",
    response_model=list[EmailConfigResponse],
    summary="Bulk upsert email config — IMAP_USER, IMAP_PASSWORD only",
)
...
@router.post(
    ...
    status_code=201,
    summary="Upsert severity → priority mapping (upsert by severity + tier_id)",
)
...
@router.put(
    ...
    summary="Upsert product config — min_severity and default_escalate",
)
...
@router.get(
    ...
    summary="Open tickets grouped by priority (real-time, local DB only)",
)
...
@router.get(
    ...
    summary="SLA breaches trend grouped by day (local DB only)",
)
...
@router.get(
    ...
    summary="Average and median first response time (local DB only)",
)
...
@router.get(
    ...
    summary="Ticket stats by product — product names resolved via auth-service",
)
